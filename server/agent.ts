...
Verify on ONE aerial, then commit. Once a key gives a short list, pull ONE wide swisstopo aerial over it, match roof/garden/pool/veranda, and zoom (a tighter bbox) only into the best 1–2. Read the exact street + number from the register (GWR). When the key + the aerial + the register agree, that is the answer — submit and stop. If you're on the right cluster but can't spot the target by eye, STOP re-looking — pull the buildings' addresses from GWR and score each against the listing; the answer is very often one you already glanced past.
