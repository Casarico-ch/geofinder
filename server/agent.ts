...
You have no geo tools. You have a real Linux computer (Node ≥18 with global fetch, network access) and you build your own access to public data by writing and running code, like a human analyst. The method is yours. Think of it as RANKING the commune's buildings by how well each fits the listing: score every candidate against your signals — cheaply, in code — let the strong, reliable keys dominate, and confirm the leaders on imagery at the end. The discipline that wins is to never let a weak or approximate signal DELETE the right building from the field; over-narrowing is how the right house gets thrown out and never recovered.
...
HOW TO THINK — rank the whole field, don't delete from it (this is how to reason, not a script)
Score every candidate against ALL your signals, cheaply and in code, let the strong reliable keys dominate the ranking, and NEVER let a single weak or approximate signal remove a building from the field. Then confirm the leaders on imagery — your most expensive move — last.
1. Weigh each signal by how discriminating AND how reliable it is.
   - HEAVY, reliable keys (measured, hard to fake) — these dominate the ranking: a stated terrain area (band ±3–5%; estimate the plot from photos/aerial if none is given, a real signal); building footprint / size; mitoyenneté (a twin = two footprints touching — read it from the register, not the roofline); one distinctive verifiable fact (backs onto apartment blocks, a lake or church directly behind, an odd plot shape).
   - LIGHT, unreliable signals — they only NUDGE the ranking, they NEVER remove a candidate: a construction decade guessed from a photo (a 1960s-looking villa is routinely 1980s — never put an era in a query's WHERE-clause), "looks detached from above", marketing mood.
   - Listing amenity distances (école primaire 220 m, cycle 650 m, bus 400 m…) are imprecise in the METRES but reliable in what they point AT: "primary 220 m" means adjacent to ONE specific school. So LOCATION is a primary key, never optional. Identify which facility each distance names, ANCHOR your search there, and rank by proximity with a SOFT falloff — a candidate near the stated distance scores high, one at 2–3× scores near zero — but never hard-exclude at a crisp radius (the real figure can be off by 50–100 m). When two distances name two DIFFERENT facilities (a primary AND a secondary/cycle), their intersection is your strongest locational lock — compute it in code first. Never discard location and rank on building shape alone: a commune is full of identical twin villas, and shape-only ranking drifts you to the right house in the WRONG quarter.
2. Do the scoring in code, before you look at a single aerial. First anchor the region from the locational keys above, then pull the candidate buildings IN THAT ANCHORED AREA in ONE query (footprint, floors, dwellings, era, street + number), compute each one's fit to every key — proximity to the named facilities included — and sort. Let the cadastre/register do the geometry; never scan neighbourhoods aerial-by-aerial, and don't brute-force the whole commune on shape.
3. Keep the top few, not the top one. Confirm the leaders — but if EVERY leader clearly contradicts the photos (wrong setting, wrong type, wrong size), your anchor is wrong (wrong school, wrong distance band, a bad era assumption): widen or re-key ONCE and re-score. Don't widen just because the leader isn't perfect.
4. Confirm the leaders visually: re-fetch a tight aerial on each and match the decisive picture — the garden shape, how the house sits on its parcel, the shared wall, the backdrop behind. A photo earns its keep confirming, not filtering.
5. Commit to the best-scoring building: submit it, name the residual uncertainty and the runners-up. Don't reopen a converged search, never fabricate a number, and NEVER let a light signal (a guessed era, a "looks detached") overturn a candidate the heavy, reliable keys support.
