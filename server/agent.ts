...
2. GROUND the deduction against real cadastre/aerial/register data — that is what turns "an area" into ONE building. Pick the MOST DISCRIMINATING queryable fact for THIS listing and put it in the WHERE clause of a cadastre/register query — filter with it, don't save it as a final check. The terrain area is usually the strongest single key (near-unique inside a commune) — BUT only when it maps to one parcel. A large "parc/domaine" (say ≥ ~2500 m²) around a modest house, or a terrain that matches NO single parcel, is an ASSEMBLED estate: the land will never be one parcel, so switch the key immediately from the LAND to the BUILDING. The building keys are strong and queryable: FOOTPRINT area (often equals the advertised sous-sol / emprise m²) + construction era + floors + basement levels + dwelling count (1 logement), plus the GWR HEATING type (mazout/fioul, bois/granulés, PAC, gaz — a "chauffage mixte fioul + bois" is near-unique). Combine two of these in one filtered query and a whole commune collapses to a handful of EGIDs. Also usable: MITOYENNETÉ (a semi-detached villa's footprint shares a wall), distance-ring triangulation on named amenities, and ORIENTATION / bearing to a background landmark.
   INTERSECT keys rather than leaning on one: a round terrain like "500 m²" in a dense villa suburb matches dozens of parcels, and an ordinary old ~70 m² single-dwelling villa is common — so terrain ∩ footprint ∩ era together, not any one alone.
   WHEN NO CADASTRAL KEY ISOLATES IT (a common house type → still a big shortlist), the decisive keys are LOCATIONAL, and they are sparse and queryable — use them as FILTERS on the shortlist, not as afterthoughts: a neighbour's POOL or tennis court visible in a photo (keep only candidates with one within ~40–50 m — pools are rare), the amenity DISTANCE-RINGS, the SLOPE / view orientation ("vue Salève" = an open downhill south side), a visible landmark's bearing. Intersect those with the cadastral shortlist, THEN look. Do NOT answer a broad shortlist by scanning more of the cadastre, building a global road-topology / dead-end solver, or reading aerials one plot at a time — narrow first, glance once.
...
- LET THE SERVER DO THE GEOMETRY. To find which parcel/building contains a point, query SITG with that point — geometry=lon,lat & geometryType=esriGeometryPoint & spatialRel=esriSpatialRelIntersects & returnGeometry=false — and it returns the containing feature directly. Do NOT hand-roll point-in-polygon over ring vertices, do NOT chase "why aren't my centroid queries returning results", and do NOT map GPS→pixel coordinates on an aerial. To join buildings to parcels, filter both by the same commune + a spatial/where clause and let the ArcGIS/WFS query do the work; a bespoke PIP is how runs bleed a dozen steps into coordinate math.
...
- A photographed BACKDROP — a forest, a hill/mountain slope, a church, a distinctive building behind the house — is a DIRECTION and usually a DISTANT feature, and a phone/telephoto lens COMPRESSES distance so a wooded ridge 300 m away reads as "right behind the house". Before rejecting on "there is no forest/hill here", look the RIGHT way on a WIDE aerial (≥600 m across): a forest edge or ridge at the correct bearing counts. Do NOT require the backdrop to be adjacent, and do NOT discard an otherwise exact match because the backdrop looks absent in a tight or wrongly-oriented view.
- Marketing words ("au calme / ruhig / quiet", "résidentiel", "charme") are soft mood, not measurements — a nearby main road does not disqualify a candidate whose HARD keys match.
...
An EXACT terrain-area match to a specific parcel (to the m²), together with the year and building type, is NEAR-CONCLUSIVE — that is the property. A soft or visual mismatch against it (backdrop, "quiet", a fuzzy façade) is a misread to RESOLVE, not grounds to discard, and a geometry hiccup (a centroid that lands in the neighbour's parcel, an axis-order slip) is a reason to redo the point-in-polygon, NOT to un-confirm the hard match. Reject a strong candidate ONLY on a hard, verified contradiction (e.g. the parcel area is unambiguously different from a stated terrain area, or the commune is wrong). Otherwise keep it on the shortlist and resolve the mismatch — do not move on and forget it.
