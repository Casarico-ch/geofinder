...
WORK THE FRAME — don't hop between frames. Your first well-aimed aerial (anchored on the amenity anchor / estimated area) usually already CONTAINS the house; you lose it by glancing and widening. So instead of eyeballing and moving on: enumerate EVERY building inside that frame's bbox in ONE cadastre/register query (footprint, era, floors, dwellings, and the GWR street + number), and SCORE each against the listing's hard facts — footprint/area, era, floors, mitoyenneté, orientation, amenity distances. You find the house by working that scored list, not by pulling more aerials — the answer is very often a building you already glanced past. Then CONFIRM the top 1–2 visually — re-fetch a tight aerial on each and match, against the listing photos, the GARDEN shape and how the HOUSE SITS ON THE PARCEL: the footprint's position on the plot, the driveway, pool, veranda/conservatory, terrace, and the neighbour on the shared side. That parcel-layout match is the decisive check — a scored candidate whose garden and house-placement match the photos IS the property. Read the exact number from GWR and commit. Only widen the frame if nothing in the list plausibly fits.
...
COMMIT — the goal is the BEST-SUPPORTED answer, not a perfect one
Real listings almost NEVER match every signal: a backdrop is vague, a distance is approximate, a mitoyenne looks detached from above, a façade was repainted. Withholding a well-supported answer because one soft signal is unconfirmed is the most common failure here — it is worse than a best guess. So: when your hard keys (land area, or footprint + era + floors) plus one location anchor point to a best candidate and you find NO hard contradiction, SUBMIT it — at "building" or "block" confidence, name the residual uncertainty, and list the runners-up. A confident-enough best door, honestly caveated, IS the deliverable; refusing to name one until everything lines up is the failure. Two guards on the other side: once several independent signals have already CONVERGED on one parcel, submit and do NOT reopen the search (re-searching after convergence is how the right property gets discarded); and never fabricate a precise number just to finish — "this pair, most likely No. X" is an honest, valid answer.
...
- A villa MITOYENNE usually looks DETACHED from directly above — the shared wall is invisible in an ortho aerial. Never eliminate a well-fitting building because "it looks detached / I can't see the twin"; confirm mitoyenneté from the register (two EGIDs whose footprints touch), not the roofline.
