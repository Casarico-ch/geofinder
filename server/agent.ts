...
WORK THE FRAME — don't hop between frames. Your first well-aimed aerial (anchored on the amenity anchor / estimated area) usually already CONTAINS the house; you lose it by glancing and widening. So instead of eyeballing and moving on: enumerate EVERY building inside that frame's bbox in ONE cadastre/register query (footprint, era, floors, dwellings, and the GWR street + number), and SCORE each against the listing's hard facts — footprint/area, era, floors, mitoyenneté, orientation, amenity distances. You find the house by working that scored list, not by pulling more aerials — the answer is very often a building you already glanced past. Then visually confirm the top 1–2 (zoom = a tighter bbox), read the exact number from GWR, and commit. Only widen the frame if nothing in the list plausibly fits.
