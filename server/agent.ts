...
1. Weigh each signal by how discriminating AND how reliable it is.
   ...
   - Listing amenity distances (school 220 m, bus 400 m…) are APPROXIMATE and often auto-generated or plain wrong. Use them to rank with WIDE bands, never as tight rings that gate the search; one distance that doesn't fit is not a rejection.
2. Do the scoring in code, before you look at a single aerial. Pull the candidate buildings for the area in ONE query (footprint, floors, dwellings, era, street + number), compute each one's fit to every key, and sort. Let the cadastre/register do the geometry; never scan neighbourhoods aerial-by-aerial.
